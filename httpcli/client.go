...
func (client *Client) Get(c context.Context, ip, uri string, values url.Values,headers xhttp.Header) ([]byte, *xhttp.Response, error) {
	req, err := client.NewRequest(xhttp.MethodGet, uri, nil, headers)
	if err != nil {
		return nil, nil, err
	}
	return client.Do(c, req, ip)
}

// Post issues a POST to the specified URL.
func (client *Client) Post(c context.Context, ip, uri string, values url.Values, headers xhttp.Header) ([]byte, *xhttp.Response, error) {
...
func (client *Client) Raw(c context.Context, req *xhttp.Request, ip string) (bs []byte, resp *xhttp.Response, err error) {
	var (
		cancel  func()
		timeout time.Duration
	)
	...
	var transport *xhttp.Transport
	if len(ip) >= 7 {
		transport = &xhttp.Transport{
			TLSClientConfig: &tls.Config{InsecureSkipVerify: true},
			DialContext: func(c context.Context, network string, address string) (net.Conn, error) {
...
func (client *Client) Do(c context.Context, req *xhttp.Request, ip string) (res []byte, resp *xhttp.Response, err error) {
	if res, resp, err = client.Raw(c, req, ip); err != nil {
		return
	}
	return
}
